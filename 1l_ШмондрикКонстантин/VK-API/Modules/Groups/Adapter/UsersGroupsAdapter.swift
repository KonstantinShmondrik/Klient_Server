//
//  UsersGroupsAdapter.swift
//  VK_ШмондрикКонстантин
//
//  Created by Константин Шмондрик on 19.03.2022.
//
...
final class UsersGroupsAdapter {
        ...
        func getUsersGroups() -> Promise<[UsersGroups]>{
            return Promise<[UsersGroups]> {seal in
                    ...
                    if let jsonData = response.data {
                        ...
                        do {
                            ...
                            for groups in groups {
                                userGroup.append(self.usersGroup(from: groups))
                            }
                            
                            seal.fulfill(userGroup)
                        } catch {
                            seal.reject(error)
                        }
                    }
     ...
     func usersGroup(from groups: UsersGroupsDAO) -> UsersGroups {
        return UsersGroups(id: groups.id,
                           isClosed: groups.isClosed,
                           isAdvertiser: groups.isAdvertiser,
                           type: groups.type,
                           isMember: groups.isMember,
                           photo50: groups.photo50,
                           photo200: groups.photo200,
                           activity: groups.activity,
                           isAdmin: groups.isAdmin,
                           photo100: groups.photo100,
                           name: groups.name,
                           screenName: groups.screenName)
    }
